...
class BubbleSheetScanner:
    ...
    def extract_quiz_title(self, ocr_text):
        """Extract quiz title from OCR text."""
        try:
            lines = ocr_text.split('\n')
            for line in lines[:3]:  # Check first 3 lines for title
                line = line.strip()
                if line and not line.lower().startswith(('name:', 'student', 'id:')):
                    return line
        except Exception as e:
            print(f"Error extracting quiz title: {e}")
        return None
    ...
    def extract_student_info(self, ocr_text):
        try:
            lines = ocr_text.split('\n')
            ...
            for line in lines:
                ...
                if 'name:' in line:
                    student_info['name'] = line.split('name:')[1].strip()
                elif 'id:' in line:
                    student_info['id'] = line.split('id:')[1].strip()
    ...
    def get_correct_answers(self, question_count=None, quiz_title=None):
        """Get correct answers for a specific quiz or fallback to default pattern."""
        try:
            from app import db
            from models import Quiz

            # First try to find quiz by title with exact match or partial match
            if quiz_title:
                quiz = Quiz.query.filter(Quiz.title.ilike(f"%{quiz_title}%")).first()
                if quiz:
                    print(f"Found quiz: {quiz.title} with answer key: {quiz.answer_key}")
                    # Extract answers from QR code if available
                    try:
                        ...
                        for obj in decoded_objects:
                            if obj.type == 'QRCODE':
                                ...
                                if 'answers:' in data:
                                    answer_key = data.split('answers:')[1].split(';')[0].strip().upper()
                                    answers = {}
                                    for i, answer in enumerate(answer_key, 1):
                                        answers[str(i)] = answer
                                    print(f"Using QR code answers: {answers}")
                                    return answers
                    except Exception as e:
                        print(f"Error extracting answers from QR: {e}")

                    # Fallback to quiz lookup
                    answers = {}
                    answer_key = quiz.answer_key.strip().upper()
                    for i, answer in enumerate(answer_key, 1):
                        answers[str(i)] = answer
                    print(f"Using quiz answers: {answers}")
                    return answers

            # Fallback to database answers if no quiz found
    ...
    def process_sheet(self, image_path):
        ...
        try:
            ...
            # Extract quiz title and student info using OCR
            ocr_text = self.extract_text_from_image(image_path)
            quiz_title = self.extract_quiz_title(ocr_text)
            ocr_student_info = self.extract_student_info(ocr_text)

            print(f"Detected quiz title: {quiz_title}")

            # Get correct answers based on quiz title
            correct_answers = self.get_correct_answers(question_count, quiz_title)
    ...
    def extract_student_info_from_image(self, image_path):
        """
        Extract student information from QR code, fallback to OCR if no QR found.
        ...
        try:
            ...
            for obj in decoded_objects:
                if obj.type == 'QRCODE':
                    try:
                        # Expect QR data in format "name:John Doe;id:12345-S"
                        data = obj.data.decode('utf-8')
                        print(f"QR Code detected! Raw data: {data}")
                        info = dict(item.split(':') for item in data.split(';'))
                        print(f"Extracted student info from QR: Name={info.get('name', '')}, ID={info.get('id', '')}")
                        return {
                            'name': info.get('name', ''),
                            'id': info.get('id', '')
                        }
            ...
            # Fallback to OCR if no valid QR code found
    ...
    def _parse_student_info(self, ocr_text):
        ...
        for line in lines:
            ...
            if "-S" in line:
                ...
                student_name = None
                words = line.split()
                # First strictly check for student ID format (XXXXXXXX-S)
                if any(w for w in words if "-S" in w and w.replace("-S", "").isdigit()):
                    student_id = next(w for w in words if "-S" in w).strip()
                    # Get the name by removing the ID part and any extra spaces
                    remaining_text = line.replace(student_id, "").strip()
                    if remaining_text.startswith("_"):
    ...
    def _find_filled_bubble(self, roi):
        ...
        for i in range(num_options):
            ...
            for contour in contours:
                ...
                cv2.drawContours(mask, [contour], -1, 255,-1, 255, -1)
